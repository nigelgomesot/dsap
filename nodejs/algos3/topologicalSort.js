...
const topologicalSort = graph => {
  ...
  for (let vertex = 0; vertex < graph.nodeCount; vertex++) {
    if (!visited[vertex])
      time = topologicalSortUtil(graph, visited, departures, time, vertex)
  }

  for (let vertex = 0; vertex < graph.nodeCount; vertex++) {
    const vertexIndex = departures.indexOf(Math.max(...departures))
    departures.splice(vertexIndex, 1, -1)
...
const topologicalSortUtil = (graph, visited, departures, time, vertex) => {
  visited[vertex] = true
  time++
  ...
  for (const edge of edges) {
    if (!visited[edge.destination])
      time = topologicalSortUtil(graph, visited, departures, time, edge.destination)
  }
